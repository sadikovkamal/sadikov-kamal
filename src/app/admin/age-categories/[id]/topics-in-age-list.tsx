...
/**
 * Topic list scoped to an age category. Each link drills into the topic
 * detail page (`/admin/topics/[id]`) with the age-category context preserved
 * via the `ageCategory` URL param.
 */
export function TopicsInAgeList({
  ageCategoryId,
  topics,
}: {
    ...
    return (
      <div className="rounded-lg border bg-card py-16 px-6 text-center space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Hozircha bu toifaga taglangan masalalar mavjud emas.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border bg-card overflow-hidden divide-y">
      {filtered.map((t) => (
        <Link
          key={t.id}
          href={`/admin/topics/${t.id}?ageCategory=${ageCategoryId}`}
